    ...
    <div class="col-md-7 col-lg-8">
      ...
      <form class="needs-validation" novalidate >
        <div class="row g-3">
          ...
          <div class="col-md-12">
            <label for="gwas1_input2" class="form-label">SNP location information</label>
            <input id="gwas1_input2" class="form-control" v-model="selected2" placeholder="SNP">
            <span v-if="checkinputback" class="invalid-feedback" style="display:block;">SNP location information不能为空</span>
          </div>
        </div>
        <hr class="my-4">
...
export default defineComponent({
  ...
  setup () {
    ...
    const checkinputback = ref(false)
    ...
    const selected2 = ref('')
    const datasearch = ()=>{
      if(selected2.value.trim()==='' || !check.value)
      {
        if(!check.value){
        ...
        if(selected2.value.trim()==='' ){
          checkinputback.value=true
        }else{
          checkinputback.value=false
        }
      }else {
        checkback.value=false
        checkinputback.value=false
